...
function EditAddress() {
  ...
  const [districts, setDictrics] = useState(currentAddress.state);
  ...
  return (
    <div className="flex flex-col items-center w-full h-full gap-4 scroll-auto mb-4 ">
      <div className="font-bold text-center mt-2">Trang chỉnh sửa địa chỉ </div>
      <div className="w-[calc(100%-50px)]  border-slate-300 border-2 rounded-md flex flex-col divide-y-2 ">
        ...
        <form className="">
          ...
          <div className="mt-4  mx-4 ">
            ...
            >
              <option  value={currentAddress.city} selected  hidden>{currentAddress.city}</option>
              {cityList.length > 0 ? (
                cityList.map((data) => (
          ...
          <div className="mt-4  mx-4 ">
            ...
            <select
              ...
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  block w-full p-2.5"
            >
              <option  value={currentAddress.state} selected  hidden>{currentAddress.state}</option>
...
{isDone? <p className="m-2 mb-0 ml-4">Thông Tin cập nhật Thành Công !</p>:''}
          <button
